...
The process of a major or minor upgrade is performed in several steps and it is helpful to remember them 

. Patching the current system
. Starting the upgrade process
. Merging configuration files
. Committing changes to the kernel and modules
. Rebooting
. Committing changes to "userland" (binaries) and configuration files
. Upgrading installed software (optional)
. Removing old libraries

==== Preparing the current installation

Before upgrading to a new version, it is important to ensure the existing FreeBSD installation is up to date with respect to security and errata patches.
Running the following commands will update the current installation to the latest _patch_ release available for it:
...
==== Starting a major or minor upgrade

After ensuring that the current installation is patched, the upgrade can begin.
The first step in the upgrade is getting `freebsd-update` to begin the upgrade by issuing an "upgrade" command and specifying the desired operating system version to upgrade to.
...
At this stage `freebsd-update` will evaluate the configuration file and current system in an attempt to gather the information necessary to perform the upgrade.
...
==== Performing the merge interview

After `freebsd-update` has downloaded the binary patches required for the upgrade and applied them to a staging directory,
it will then begin an interview process with the user to help manage one of the more difficult phases of an upgrade: upgrading
the system configuration files. In the lifetime of a typical system the system configuration files start out at their defaults
when the system is first installed but are often modified, over time, to fit the needs of the system administrator and
accomodate additional installed software. The user often wishes to preserve these changes, 
...
==== Commiting kernel changes

NEEDS REWORK: Clarify that this step is _kernel only_:

At this stage the system has not yet been altered. Instead, all patching and merging for the upgrade has been performed in a separate, staging directory.
To continue the upgrade these staging files will need to be committed to their final locations on disk.
Assuming that all patches have been applied successfully and all configuration files have been merged, the changes can be committed to disk using the following command:
