...
Before upgrading to a new version, it is important to ensure the existing FreeBSD installation is up to date with respect to security and errata patches. Running the
follwing commands will update the current installation to the latest _patch_ release available for it:
...
After ensuring that the current installation is patched, the upgrade can begin. The following command, when run on a FreeBSD 13.1 system, will upgrade it to FreeBSD 13.2:
